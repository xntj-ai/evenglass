defmodule EvenglassWeb.PCUserLive.ConfirmationTest do
  ...
  describe "Confirm pc_user" do
    ...
    test "consumes the magic link once and gates on TOTP", %{
      conn: conn,
      unconfirmed_pc_user: pc_user
    } do
      token =
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :info) =~
               "Pc user confirmed successfully"

      assert Accounts.get_pc_user!(pc_user.id).confirmed_at
      # First factor done, but full session is gated on TOTP.
    ...
    test "magic-link login of confirmed pc_user gates on TOTP", %{
      conn: conn,
      confirmed_pc_user: pc_user
    } do
      token =
